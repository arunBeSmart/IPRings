...
<table class='table'>
    ...
    <td width='60%'><input type="text" id='barcodeScanIn' class='form-control' autofocus onchange='getBinStockToInvoiceDirect( "{{$INVOICE->CUSTOMER_PART_NUMBER}}",this.value);'>
    </td></tr></table>
...
$yieldQty=$scannedQty=0;
foreach($STOCK as $record)
{ $yieldQty=$yieldQty+$record->yield_qty;
 $scannedQty=$scannedQty+$record->SCANNED_QTY;
$IPR_REF=$record->IPR_REF; } /*
echo "<br>Yield Qty:".$yieldQty;
echo "<br>Scanned Qty:".$scannedQty;
echo "<br>Invoice Qty:".$BILLED_QTY;
echo "<br>IPR_REF:".$IPR_REF; */
echo "<input type='hidden' id='IPR_REF' name='IPR_REF' value='".$IPR_REF."'>";
if($BILLED_QTY>($yieldQty-$scannedQty))
{
    echo "<script>alert('Insufficent Work Order QTY!!');$('#myModal').modal('hide');</script>";
}
?>
  <tbody>
        @foreach($STOCK as $record)
...
<br><div id='alertDiv'></div>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-danger" data-dismiss="modal"
            ...
            <button type="submit" disabled=1 id='saveButton' class="btn btn-success" onclick="confirmMappStkToInvoiceDirect();"><font size='+1'>SAVE</font></button>
            </td></tr></table>
...
<script>
    ...
    function confirmMappStkToInvoiceDirect()
    {
        BILLED_QTY=$('#BILLED_QTY').val();
            ...
            if(HEAT_CODE!=0 && HEAT_CODE!='')
                        {
                            ...
                            var completeurl = url +'/confirmMappStkToInvoiceDirect';
...
function getBinStockToInvoiceDirect(CUSTOMER_PART_NO,barcodeScanIn)
{
    
    $('#barcodeScanIn').focus();
    $('#barcodeScanIn').val('');
    var INVOICE_ID=$('#INVOICE_ID').val();
    var IPR_REF=$('#IPR_REF').val();
    BILLED_QTY=$('#BILLED_QTY').val();
    data = 'barcodeScanIn='+barcodeScanIn+'&CUSTOMER_PART_NO='
    +CUSTOMER_PART_NO+'&IPR_REF='+IPR_REF +'&INVOICE_ID='+INVOICE_ID+'&BILLED_QTY='+BILLED_QTY;


                var completeurl = url +'/getBinStockMappedToInvoiceDirect';
                var type = "POST";
                var place = 'alertDiv';
                ajaxload(type, completeurl, data, place);
...
$('#barcodeScanIn').focus();
getBinStockToInvoiceDirect( "{{$INVOICE->CUSTOMER_PART_NUMBER}}",'');
</script>
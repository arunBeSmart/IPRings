...
<br>
<input type="text" id='barcodeScanIn' class='form-control' autofocus onchange='getBinStockToInvoice( "{{$INVOICE->CUSTOMER_PART_NUMBER}}",this.value);'>
<br>
...
if ($BILLED_QTY >= $STOCK and $record->LOCKED!=1) {
    $BILLED_QTY = $BILLED_QTY - $STOCK;
    $ASSIGNED_QTY = $ASSIGNED_QTY + $STOCK;

    echo "<input name='assigned[]' id='assigned" . $record->id . "' onchange='getTotal(" . $record->id . ");' class='form-control assigned' id='' name='' value='" . $STOCK . "'>";

} else if ($BILLED_QTY == 0 and $record->LOCKED!=1) {
    echo "<input name='assigned[]' id='assigned" . $record->id . "'  onchange='getTotal(" . $record->id . ");'  class='form-control assigned' id='' name='' value='0'>";

} else if ($BILLED_QTY < $STOCK and $record->LOCKED!=1) {
    $ASSIGNED_QTY = $ASSIGNED_QTY + $BILLED_QTY;
    echo "<input name='assigned[]' id='assigned" . $record->id . "'  onchange='getTotal(" . $record->id . ");'  class='form-control assigned' id='' name='' value='" . $BILLED_QTY . "'>";
    $BILLED_QTY = 0;
}
if( $record->LOCKED==1)
{
...
?>
    ...
    <th  colspan='4'><input type="text" name="SUPPLIER" id="SUPPLIER" class='form-control'></th>
        </tr>
    ...
    <th  colspan='4'><input type="text" name="REV_LEVEL" id="REV_LEVEL" class='form-control'></th>
        </tr>
        <tr><th colspan='3'>DATE_MFG<font color='red'>*</font></th>
    <th  colspan='4'><input type="text" name="DATE_MFG" id="DATE_MFG" class='form-control'></th>
        </tr>
        <tr><th colspan='3'>DATE_SHIPPED<font color='red'>*</font></th>
    <th  colspan='4'><input type="text" name="DATE_SHIPPED" id="DATE_SHIPPED" class='form-control'></th>
        </tr>
        <tr><th colspan='3'>FROM_ADDRESS<font color='red'>*</font></th>
    <th  colspan='4'>
        <textarea  name="FROM_ADDRESS" id="FROM_ADDRESS" class='form-control' rows="4" cols="50">
IP RINGS LIMITED,
...
Tamil Nadu-603 209, India</textarea></th>
    ...
    <th  colspan='4'><textarea  name="TO_ADDRESS" id="TO_ADDRESS" class='form-control' rows="4" cols="50">
